...
class TestBufLintCompliance:
    ...
    **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 1.5, 2.1, 2.2, 2.3, 2.4, 2.5**
    ...
    @pytest.mark.skipif(
        not (PROTOS_DIR / "buf.yaml").exists(),
        reason="buf.yaml not found"
    )
    ...
    **Validates: Requirements 9.1, 9.3**
    ...
    def test_valid_version_suffixes_match_pattern(self, version: str) -> None:
        ...
        assert self.VERSION_PATTERN.search(package_name), (
            f"Package {package_name} should match version pattern"
        )
    ...
    def test_all_packages_have_version_suffix(self) -> None:
        ...
        for proto_file in proto_files:
            ...
            if match:
                ...
                assert self.VERSION_PATTERN.search(package_name), (
                    f"Package '{package_name}' in {proto_file.name} "
                    "should end with version suffix (e.g., .v1)"
                )
...
class TestPaginationStructure:
    ...
    **Validates: Requirements 7.1, 7.2, 7.3, 7.4**
    ...
    def test_list_items_request_uses_pagination(self) -> None:
        ...
        assert "common.v1.PageRequest" in content, (
            "ListItemsRequest should use common.v1.PageRequest"
        )
    ...
    def test_list_items_response_uses_pagination(self) -> None:
        ...
        assert "common.v1.PageResponse" in content, (
            "ListItemsResponse should use common.v1.PageResponse"
        )
...
class TestTimestampWellKnownTypes:
    ...
    **Validates: Requirements 4.1, 4.3, 5.4**
    ...
    def test_errors_proto_uses_timestamp_type(self) -> None:
        ...
        assert self.TIMESTAMP_IMPORT in content, (
            "errors.proto should import google/protobuf/timestamp.proto"
        )
        assert self.TIMESTAMP_TYPE in content, (
            "errors.proto should use google.protobuf.Timestamp type"
        )
    ...
    def test_items_proto_uses_timestamp_type(self) -> None:
        ...
        assert self.TIMESTAMP_IMPORT in content, (
            "items.proto should import google/protobuf/timestamp.proto"
        )
        
        # Check that time fields use Timestamp type
        assert "google.protobuf.Timestamp create_time" in content, (
            "create_time should be google.protobuf.Timestamp"
        )
        assert "google.protobuf.Timestamp update_time" in content, (
            "update_time should be google.protobuf.Timestamp"
        )
...
class TestFieldMaskUpdateSemantics:
    ...
    **Validates: Requirements 8.1, 8.2, 8.3**
    ...
    def test_update_item_request_has_field_mask(self) -> None:
        ...
        assert self.FIELD_MASK_IMPORT in content, (
            "items.proto should import google/protobuf/field_mask.proto"
        )
        assert self.FIELD_MASK_TYPE in content, (
            "UpdateItemRequest should include google.protobuf.FieldMask"
        )
        assert "update_mask" in content, (
            "UpdateItemRequest should have update_mask field"
        )
...
class TestProtovalidateValidation:
    ...
    **Validates: Requirements 3.1, 3.2, 3.3, 3.4**
    ...
    def test_items_proto_has_validation(self) -> None:
        ...
        assert self.VALIDATE_IMPORT in content, (
            "items.proto should import buf/validate/validate.proto"
        )
        assert self.VALIDATE_ANNOTATION in content, (
            "items.proto should use buf.validate annotations"
        )
    ...
    def test_errors_proto_has_validation(self) -> None:
        ...
        assert self.VALIDATE_IMPORT in content, (
            "errors.proto should import buf/validate/validate.proto"
        )
    ...
    def test_pagination_proto_has_validation(self) -> None:
        ...
        assert self.VALIDATE_IMPORT in content, (
            "pagination.proto should import buf/validate/validate.proto"
        )
    ...
    def test_page_size_validation_range(self, page_size: int) -> None:
        ...
        assert "lte: 100" in content or "lte:100" in content, (
            "page_size should have max validation of 100"
        )
    ...
    **Validates: Requirements 10.1, 10.2, 10.3**
    ...
    def test_item_has_name_field(self) -> None:
        ...
        assert "string name = 2" in content, (
            "Item message should have name field at position 2"
        )
    ...
    def test_item_name_has_pattern_validation(self) -> None:
        ...
        assert "pattern" in content, (
            "Item name field should have pattern validation"
        )
        assert "items/" in content, (
            "Item name pattern should include 'items/' collection prefix"
        )
    ...
    def test_resource_name_format_with_uuid(self, uuid: Any) -> None:
        ...
        assert pattern.match(resource_name), (
            f"Resource name '{resource_name}' should match AIP-122 pattern"
        )
    ...
    **Validates: Requirements 5.5**
    ...
    def test_error_code_enum_exists(self) -> None:
        ...
        assert "enum ErrorCode" in content, (
            "errors.proto should define ErrorCode enum"
        )
    ...
    def test_error_code_has_unspecified_zero_value(self) -> None:
        ...
        assert "ERROR_CODE_UNSPECIFIED = 0" in content, (
            "ErrorCode should have ERROR_CODE_UNSPECIFIED = 0"
        )
    ...
    def test_error_code_values_have_prefix(self) -> None:
        ...
        for code in expected_codes:
            assert code in content, (
                f"ErrorCode should include {code}"
            )
