...
class TestValidationConsistencyProperties:
    ...
    **Validates: Requirements 5.2**
    ...
    def test_valid_input_always_passes(self, name: str, age: int, email: str) -> None:
    ...
    @given(name=valid_name, age=valid_age, email=valid_email)
    @settings(max_examples=100)
    def test_validation_is_deterministic(self, name: str, age: int, email: str) -> None:
    ...
    @given(
        code=st.text(min_size=1, max_size=20).filter(lambda x: not x.isupper() or len(x) != 8 or "-" not in x),
        score=st.floats(min_value=0.0, max_value=100.0, allow_nan=False),
    )
    @settings(max_examples=50)
    ...
    @given(name=valid_name, age=valid_age, email=valid_email)
    @settings(max_examples=50)
    def test_validation_idempotent(self, name: str, age: int, email: str) -> None:
