...
valid_string_id = st.text(alphabet=st.sampled_from("0123456789ABCDEFGHJKMNPQRSTVWXYZ"), min_size=26, max_size=26)
...
class TestEntityCreationProperties:
    ...
    **Validates: Requirements 2.4, 2.5**
    ...
    @given(created_by=valid_user_id, updated_by=valid_user_id)
    @settings(max_examples=100)
    def test_auditable_entity_preserves_audit_fields(self, created_by: str, updated_by: str) -> None:
        """AuditableEntity should preserve audit fields.

        *For any* valid audit data, creating an entity should preserve it.
        """
        entity = AuditableEntity[str](created_by=created_by, updated_by=updated_by)
    ...
    @settings(max_examples=100)
    def test_entity_preserves_deleted_flag(self, entity_id: str, is_deleted: bool) -> None:
