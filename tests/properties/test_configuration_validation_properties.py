...
class TestConfigurationValidationProperties:
    ...
    **Validates: Requirements 3.5**
    ...
    def test_valid_config_always_loads(self, app_name: str, port: int, debug: bool) -> None:
        """Valid configuration should always load successfully.

        *For any* valid configuration values, loading should succeed.
        """
        config = TestConfig(app_name=app_name, port=port, debug=debug)
    ...
    @given(app_name=st.text(min_size=1, max_size=50).filter(lambda x: x.strip()))
    @settings(max_examples=50)
    ...
    def test_optional_field_can_be_none(self, app_name: str, port: int) -> None:
        ...
        *For any* valid config, optional fields can be None.
        """
        config = TestConfig(app_name=app_name, port=port, api_key=None)
    ...
    @settings(max_examples=50)
    def test_optional_field_accepts_value(self, app_name: str, port: int, api_key: str) -> None:
        """Optional fields should accept values.

        *For any* valid config with optional field set, it should be stored.
        """
        config = TestConfig(app_name=app_name, port=port, api_key=api_key)
    ...
    @settings(max_examples=50)
    def test_required_fields_must_be_present(self, database_url: str, secret_key: str) -> None:
        """Required fields must be present.

        *For any* valid required field values, config should load.
        """
        config = RequiredFieldsConfig(database_url=database_url, secret_key=secret_key)
    ...
    @settings(max_examples=50)
    def test_default_values_can_be_overridden(self, database_url: str, secret_key: str, api_version: str) -> None:
        """Default values should be overridable.

        *For any* valid config, default values can be overridden.
        """
        config = RequiredFieldsConfig(database_url=database_url, secret_key=secret_key, api_version=api_version)
    ...
    @settings(max_examples=50)
    def test_default_values_used_when_not_provided(self, database_url: str, secret_key: str) -> None:
