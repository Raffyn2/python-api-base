    ...
    for ns in namespaces:
        ...
        if ns["metadata"]["name"] == "my-api":
            assert injection == "enabled", (
                f"Namespace {ns['metadata']['name']} should have istio-injection=enabled"
            )
        # istio-system should have injection disabled
        elif ns["metadata"]["name"] == "istio-system":
            assert injection == "disabled", (
                "istio-system namespace should have istio-injection=disabled"
            )
    ...
    for op in operators:
        ...
        if pilot_resources:
            ...
            if "cpu" in limits:
                ...
                assert 500 <= cpu_mc <= 2000, (
                    f"Pilot CPU limit {limits['cpu']} not in range 500m-2000m"
                )
            if "memory" in limits:
                mem_mi = parse_memory_to_mi(limits["memory"])
                assert 512 <= mem_mi <= 2048, (
                    f"Pilot memory limit {limits['memory']} not in range 512Mi-2Gi"
                )
...
def test_mtls_strict_mode_enforcement():
    ...
    for pa in peer_auths:
        ...
        assert mtls_mode == "STRICT", (
            f"PeerAuthentication in {namespace} should have STRICT mTLS, got {mtls_mode}"
        )
    ...
    for vs in virtual_services:
        ...
        for route in http_routes:
            ...
            if len(destinations) > 1:
                ...
                total = sum(weights)
                assert total == 100, (
                    f"VirtualService route weights sum to {total}, expected 100"
                )
    ...
    for dr in dest_rules:
        ...
        if outlier:
            ...
            assert 0 <= max_ejection <= 100, (
                f"maxEjectionPercent {max_ejection} not in range 0-100"
            )
        ...
        for route in http_routes:
            ...
            if retries:
                ...
                assert 1 <= attempts <= 10, (
                    f"Retry attempts {attempts} not in range 1-10"
                )
                
                timeout = retries.get("perTryTimeout", "")
                if timeout:
                    assert re.match(r"^\d+[smh]$", timeout), (
                        f"Invalid perTryTimeout format: {timeout}"
                    )
        ...
        for route in http_routes:
            ...
            if timeout:
                ...
                assert 0 < seconds <= 300, (
                    f"Timeout {timeout} ({seconds}s) not in range 1-300s"
                )
    ...
    for gw in gateways:
        ...
        for server in servers:
            ...
            if port.get("protocol") == "HTTPS":
                tls = server.get("tls", {})
                assert tls.get("mode") in ["SIMPLE", "MUTUAL"], (
                    "HTTPS server must have TLS mode SIMPLE or MUTUAL"
                )
                assert tls.get("credentialName"), (
                    "HTTPS server must have credentialName for TLS certificate"
                )
    ...
    for vs in virtual_services:
        ...
        for gw_name in vs_gateways:
            if gw_name in gateway_hosts:
                ...
                assert match_found, (
                    f"VirtualService hosts {vs_hosts} don't match "
                    f"Gateway {gw_name} hosts {gw_hosts}"
                )
...
def test_tracing_sampling_rate_valid():
    ...
    for op in operators:
        ...
        assert 0.0 <= sampling <= 100.0, (
            f"Tracing sampling rate {sampling} not in range 0-100"
        )
        ...
        for rule in rules:
            ...
            for source in from_sources:
                ...
                for principal in principals:
                    assert principal_pattern.match(principal), (
                        f"Invalid principal format: {principal}"
                    )
...
def test_jwt_jwks_uri_present():
    ...
    for ra in request_auths:
        ...
        for rule in jwt_rules:
            ...
            assert jwks_uri or jwks, (
                "JWT rule must have either jwksUri or jwks"
            )
            
            if jwks_uri:
                assert jwks_uri.startswith("https://"), (
                    f"jwksUri must use HTTPS: {jwks_uri}"
                )
...
def test_egress_policy_registry_only():
    ...
    for op in operators:
        ...
        assert mode == "REGISTRY_ONLY", (
            f"outboundTrafficPolicy.mode should be REGISTRY_ONLY, got {mode}"
        )
