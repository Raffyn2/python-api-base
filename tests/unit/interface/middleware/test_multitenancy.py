...
spec = importlib.util.spec_from_file_location(
    "multitenancy_middleware", "src/interface/middleware/production/multitenancy.py"
)
...
class TestMultitenancyMiddleware:
    ...
    def test_middleware_without_tenant_header_not_required(self, app: FastAPI) -> None:
    ...
    def test_middleware_without_tenant_header_required(self, app: FastAPI) -> None:
        ...
        # RFC 7807 format
        data = response.json()
        assert data["title"] == "Tenant ID Required"
...
class TestTenantIsolationProperties:
    ...
    **Validates: Requirements 7.3**
    ...
    def test_different_tenants_do_not_share_context(self, tenant_a: str, tenant_b: str) -> None:
        """
        **Feature: test-coverage-80-percent-v3, Property 15: Tenant Isolation**
        **Validates: Requirements 7.3**
    ...
    @settings(max_examples=100, deadline=5000)
    def test_tenant_context_cleared_after_operation(self, tenant_id: str) -> None:
        """
        **Feature: test-coverage-80-percent-v3, Property 15: Tenant Isolation**
        **Validates: Requirements 7.3**
