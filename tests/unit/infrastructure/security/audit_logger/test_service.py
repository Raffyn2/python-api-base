...
class TestSecurityAuditLogger:
    ...
    def test_log_auth_success(self, audit_logger: SecurityAuditLogger, mock_logger: Mock) -> None:
        event = audit_logger.log_auth_success(
            user_id="test_user", client_ip="1.2.3.4", method="password"
        )
    ...
    def test_log_auth_failure(self, audit_logger: SecurityAuditLogger, mock_logger: Mock) -> None:
        event = audit_logger.log_auth_failure(
            client_ip="1.2.3.4", reason="bad password", attempted_user="test_user"
        )
        assert event.event_type == SecurityEventType.AUTH_FAILURE
        mock_logger.warning.assert_called_once()

    def test_log_authorization_denied(
        self, audit_logger: SecurityAuditLogger, mock_logger: Mock
    ) -> None:
        event = audit_logger.log_authorization_denied(
            user_id="test_user", resource="test_resource", action="read"
        )
    ...
    def test_log_rate_limit_exceeded(
        ...
        event = audit_logger.log_rate_limit_exceeded(
            client_ip="1.2.3.4", endpoint="/test", limit="10/min"
        )
    ...
    def test_log_secret_access(self, audit_logger: SecurityAuditLogger, mock_logger: Mock) -> None:
        event = audit_logger.log_secret_access(
            secret_name="test_secret", accessor="test_user"
        )
    ...
    def test_log_token_revoked(self, audit_logger: SecurityAuditLogger, mock_logger: Mock) -> None:
        event = audit_logger.log_token_revoked(
            user_id="test_user", token_jti="test_jti"
        )
    ...
    def test_log_suspicious_activity(
        ...
        event = audit_logger.log_suspicious_activity(
            client_ip="1.2.3.4", description="test activity"
        )
