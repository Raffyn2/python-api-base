...
class TestInMemoryTokenStore:
    ...
    @pytest.mark.asyncio
    async def test_store_and_get(self, store: InMemoryTokenStore, future_time: datetime) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_returns_none(self, store: InMemoryTokenStore) -> None:
        """Test getting nonexistent token returns None."""
        token = await store.get("nonexistent")
        assert token is None

    @pytest.mark.asyncio
    async def test_revoke_token(self, store: InMemoryTokenStore, future_time: datetime) -> None:
        """Test revoking token."""
        await store.store("jti-1", "user-1", future_time)
    ...
    @pytest.mark.asyncio
    async def test_revoke_nonexistent_returns_false(self, store: InMemoryTokenStore) -> None:
        """Test revoking nonexistent token returns False."""
        result = await store.revoke("nonexistent")
        assert result is False

    @pytest.mark.asyncio
    async def test_revoke_all_for_user(self, store: InMemoryTokenStore, future_time: datetime) -> None:
    ...
    @pytest.mark.asyncio
    async def test_is_valid_for_valid_token(self, store: InMemoryTokenStore, future_time: datetime) -> None:
    ...
    @pytest.mark.asyncio
    async def test_is_valid_for_revoked_token(self, store: InMemoryTokenStore, future_time: datetime) -> None:
    ...
    @pytest.mark.asyncio
    async def test_is_valid_for_nonexistent_token(self, store: InMemoryTokenStore) -> None:
...
class TestRedisTokenStore:
    ...
    @pytest.mark.asyncio
    async def test_store_token(self, store: RedisTokenStore, mock_redis: AsyncMock, future_time: datetime) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_returns_none(self, store: RedisTokenStore) -> None:
        """Test getting nonexistent token returns None."""
        token = await store.get("nonexistent")
        assert token is None

    @pytest.mark.asyncio
    async def test_is_revoked_false(self, store: RedisTokenStore, mock_redis: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_is_revoked_true(self, store: RedisTokenStore, mock_redis: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_add_to_blacklist(self, store: RedisTokenStore, mock_redis: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_returns_zero(self, store: RedisTokenStore) -> None:
        """Test cleanup returns 0 (Redis handles TTL)."""
        count = await store.cleanup_expired()
        assert count == 0

    @pytest.mark.asyncio
    async def test_is_valid_nonexistent(self, store: RedisTokenStore) -> None:
        """Test is_valid returns False for nonexistent token."""
