...
class TestMapperError:
    ...
    def test_create_with_source_and_target_types(self) -> None:
        ...
        error = MapperError("Type mismatch", source_type="UserEntity", target_type="UserDTO")
    ...
    def test_create_with_field(self) -> None:
        ...
        error = MapperError("Invalid field value", field="email")
    ...
    def test_create_with_context(self) -> None:
        ...
        error = MapperError("Validation failed", context=context)
    ...
    def test_create_with_all_parameters(self) -> None:
        ...
        error = MapperError(
            "Complete mapping error", source_type="Entity", target_type="DTO", field="id", context=context
        )

        # __str__ includes mapping context for better debugging
        assert str(error) == "Complete mapping error (Entity -> DTO) [field: id]"
    ...
    def test_can_be_raised_and_caught(self) -> None:
        ...
        with pytest.raises(MapperError) as exc_info:
            raise MapperError("Mapping failed", source_type="Source", target_type="Target")
