...
def mock_repository() -> MagicMock:
    ...
    repo.bulk_create = AsyncMock(
        return_value=BatchResult(
            succeeded=[TestItem(id="1", name="test")], failed=[], total_processed=1, total_succeeded=1, total_failed=0
        )
    )
    ...
    repo.bulk_delete = AsyncMock(
        return_value=BatchResult(succeeded=["1"], failed=[], total_processed=1, total_succeeded=1, total_failed=0)
    )
