...
class InMemoryRepository[
    ...
    **Validates: Requirements 1.2, 11.1**
    ...
    def __init__(
        self,
        entity_type: type[T],
        id_generator: Callable[[], IdType] | None = None,
    ) -> None:
        ...
        self._storage: dict[IdType, T] = {}
        self._counter = 0

    def _default_id_generator(self) -> IdType:
        ...
        """
        self._counter += 1
        return str(self._counter)  # type: ignore
    ...
    async def exists(self, id: IdType) -> bool:
        ...
        Returns:
            True if entity exists and not soft-deleted, False otherwise.
        """
    ...
    async def bulk_update(self, updates: Sequence[tuple[IdType, UpdateT]]) -> Sequence[T]:
        ...
        Returns:
            Sequence of updated entities (excludes not found).
        """
        results: list[T] = []
        for entity_id, data in updates:
            updated = await self.update(entity_id, data)
            if updated:
    ...
    async def bulk_delete(self, ids: Sequence[IdType], *, soft: bool = True) -> int:
        ...
        for entity_id in ids:
            if await self.delete(entity_id, soft=soft):
                count += 1
        return count
