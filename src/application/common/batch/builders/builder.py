...
class BatchOperationBuilder[T: BaseModel, CreateT: BaseModel, UpdateT: BaseModel]:
    ...
    def with_chunk_size(self, size: int) -> Self:
    ...
    def with_error_strategy(self, strategy: BatchErrorStrategy) -> Self:
        """Set error handling strategy."""
        self._config.error_strategy = strategy
        return self

    def with_retry(self, max_retries: int = 3) -> Self:
    ...
    def with_progress(self, callback: ProgressCallback) -> Self:
    ...
    async def create(self, items: Sequence[CreateT]) -> BatchResult[T]:
        ...
        return await self._repository.bulk_create(items, config=self._config, on_progress=self._progress_callback)

    async def update(self, items: Sequence[tuple[str, UpdateT]]) -> BatchResult[T]:
        """Execute bulk update operation."""
        return await self._repository.bulk_update(items, config=self._config, on_progress=self._progress_callback)
    ...
    async def upsert(self, items: Sequence[CreateT], *, key_field: str = "id") -> BatchResult[T]:
